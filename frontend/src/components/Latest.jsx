...
const Latest = () => {
    ...
    function increment(id){
        const updated=products.map((product)=>{
            if(product._id===id){
                const inc= {...product,defaultQuantity:product.defaultQuantity+1}
                addtocart(inc)
                return inc
            }
            else{
    ...
    function decrement(id){
        const updated=products.map((product)=>{
            if(product._id===id){
                return {...product,defaultQuantity:product.defaultQuantity-1}
            }
            else{
                return product
            }
  ...
  return (
    <div className='bg-cover pb-6 md:pb-8 my-8 md:my-16' style={{backgroundImage:`url(${whyus})`}}>
        <div className='flex items-center my-8 justify-center'>
            <h1 className='text-3xl md:text-7xl'>New Arrivals</h1>
        </div>
        
        <div className='flex flex-wrap md:gap-x-[120px] md:gap-y-[20px] md:mx-24 justify-center item-center'>
            {products.map((product,index)=>{return <div className='hover:scale-105' key={index}>
        <div className="border my-2 border-yellow-600 text-yellow-400 rounded-md md:px-4 px-3 py-2  bg-black min-w-100 md:min-w-80 md:max-w-80 w-full">
                    ...
                    <div className="font-bold">
                        {product.defaultQuantity === 0 ? (
                            ...
                            <div className="flex items-center justify-center gap-2 md:w-20 w-16 h-[34px] bg-yellow-200 rounded select-none">
                                ...
                                <button onClick={() => increment(product._id)} className="cursor-pointer text-black text-md px-2 h-full" >
                                    +
                                </button>
                            </div>
                        )}
